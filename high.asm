...
public M_pressed_on_what
public Q_pressed_on_what
...
cursor1_High db 0Ah
cursor2_High db 0eh
color_Highpl1 db 05h ;color of possible moves for pl1
Color_Highpl2 db 01h ;color of possible moves for pl2
color_Kill_Highpl1 db 0Ch ;color of killing for pl1
color_Kill_Highpl2 db 03h
;color of killing for pl2
...
Draw_border proc far   ;cx-> col dx->row  al->color ;lazem el cx we el dx kolha msh cl,dl
    ...
    push ax
    ;Get starting_pixels by mult by 100
    ...
    cmp bx,94d  ;akherha 92
    jNE loop_2maxa ;max jmp
    jmp skip_add_dia
    loop_2maxa:
    jmp loop_2a
    skip_maxloop2a:
...
Draw_border_cursor proc far   ;cx-> col dx->row  al->color ;lazem el cx we el dx kolha msh cl,dl
    ...
    cmp bx,88d   ;88 lw 3ayz tkhleha 6 msh 5 marat
    jNE loop_2max ;max jmp
    jmp skip_add_di
    loop_2max:
    jmp loop_2
    skip_maxloop2:
...
Draw_Highs  proc far
    ...
    mov al,cursor1_High ;pl1 color
    call Draw_border_cursor
    jmp finish_Draw
...
move_player_And_Delete_Old proc far
        ...
        cmp scanCode,30 ;A left
        JE Max_Jmp_A_Left
    
        cmp scanCode,17 ;W up
        JE Max_Jmp_W_Up

        cmp scanCode,32 ;D right
        JE Max_Jmp_D_Right

        cmp scanCode,31 ;S down
        JE Max_Jmp_S_Down


        cmp scanCode,48h ;arrowup
        JE Max_Jmp_Up

        cmp scanCode,50h ;arrow down
        Je Max_Jmp_down

        cmp scanCode,4Dh ;arrow Right
        Je Max_Jmp_Right

        cmp scanCode,4Bh ;arrow Left
        Je Max_Jmp_Left
...
ROW_COL_TO_Number   proc far
    ...
    mov bx, 0
    add bx,ax
    mov ah, grid[bx] ;bhot fe el ax el value ely mawgoda fe el morb3 dah(ex:white pawn) 
...
Q_Possible_Moves    proc    far  
    ...
    cmp ah,0
    JE THERE_IS_BLACK_PIECE_OR_NTH
    cmp ah,10
    JA THERE_IS_BLACK_PIECE_OR_NTH
    JMP SKP_THERE_IS_BLACK_PIECE_OR_NTH

THERE_IS_BLACK_PIECE_OR_NTH:
    call delete_all
    mov Q_pressed_on_what,0
SKP_THERE_IS_BLACK_PIECE_OR_NTH:
...
M_Possible_Moves    proc    far
    ...
    cmp ah,10
    JB THERE_IS_WHITE_PIECE_OR_NTH
    JMP SKP_THERE_IS_WHITE_PIECE_OR_NTH
    THERE_IS_WHITE_PIECE_OR_NTH:
    call delete_all
    mov M_pressed_on_what,0
SKP_THERE_IS_WHITE_PIECE_OR_NTH:
...
; TODO when pressing q on black pieces -> illigal write
